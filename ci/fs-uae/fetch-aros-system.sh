...
AROS_URL="$({ grep -oE 'href="[^"]*amiga-m68k-boot-iso[^"]*"' "$index_html" || true; } | head -n 1 | sed -e 's/^href="//' -e 's/"$//' -e 's/&amp;/\&/g')"
if [[ -z "$AROS_URL" ]]; then echo "ERROR: could not resolve $AROS_TARGET" >&2; exit 1; fi
case "$AROS_URL" in http://*|https://*) ;; //*) AROS_URL="https:${AROS_URL}" ;; /*) AROS_URL="https://aros.sourceforge.io${AROS_URL}" ;; *) AROS_URL="https://aros.sourceforge.io/${AROS_URL}" ;; esac
url_path="${AROS_URL%%\?*}"
...
case "$AROS_ARCHIVE" in *.lha|*.LHA) lha xw="$OUT_DIR/archive-extracted" "$archive" >/dev/null ;; *.zip|*.ZIP) unzip -q "$archive" -d "$OUT_DIR/archive-extracted" ;; *) echo "ERROR: unsupported archive" >&2; exit 1 ;; esac
iso="$(find "$OUT_DIR/archive-extracted" -type f \( -iname '*.iso' -o -iname '*.ISO' \) -print -quit)"
if [[ -z "$iso" ]]; then echo 'ERROR: no ISO found' >&2; exit 1; fi
